...
class QuizController extends Controller
{
    ...
    public function GetQuizQuestionAnswersApi($quiz_id, Request $request)
    {
            ...
            try{
                ...
                $questions = [];
                if($UserExam->question_array!=null) {
                    $questions_ = QuizQuestions::where('quiz_id', $quiz->id)->where('status', 1)->get();
                    $questionLast = json_decode($UserExam->question_array);
                    $i=1;
                    foreach ($questions_ as $question_key => $question_value){
                        foreach ($questionLast as $questionLastKey => $questionLastValue){
                            if($questionLastValue->id == $question_value->id){
                                array_push($questions, $question_value);
                            }
                        }
                        $i++;
                    }
                    if($i == count($questions_)):
                        foreach ($questions_ as $question_key => $question_value){
                            foreach ($questionLast as $questionLastKey => $questionLastValue){
                                foreach ($questions as $qKey => $qValue){
                                    if($questionLastValue->id != $question_value->id && $qValue->id != $question_value->id)
                                    array_push($questions, $question_value);
                                }
                            }
                        }
                    ...
                    $questions = QuizQuestions::where('quiz_id', $quiz->id)->where('status', 1)->get();
                }
                $UserExam->update(['question_array'=> json_encode($questions)]);
