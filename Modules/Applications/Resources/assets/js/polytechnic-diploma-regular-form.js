    ...
    $("#basicwizard").bootstrapWizard({
            onNext: function (t, r, a) {
                    ...
                    if(find_id=='basictab1'){
                        ...
                        if(quota==''){ 
                            error=false;
                            var quota_error= '<label id="quota-error" class="invalid-feedback" for="gender" style="display: inline-block;">This field is required.</label>';
                            $("#quota_error").html(quota_error);
                        }
...
//                    'category':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    ...
                    'last_board_study':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    'sslc_total_percentage':{required:true, normalizer: function(value) { return $.trim(value);  },number:true  },
                    'sslc_pcm_percentage':{required:true, normalizer: function(value) { return $.trim(value);  },number:true  },
                    'sslc_register_no':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    'sslc_month_year_passing':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    'sslc_subject[1]':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    'sslc_grade[1]':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    'sslc_total_percentage':{ required:true,normalizer: function(value) { return $.trim(value);  },number:true },
                    'sslc_pcm_percentage':{ required:true,normalizer: function(value) { return $.trim(value);  },number:true },
...
//                    'grade[1]':{required:true, normalizer: function(value) { return $.trim(value);  } },
                    'grand_total':{ normalizer: function(value) { return $.trim(value);  },number:true },
                    'total_percentage':{ normalizer: function(value) { return $.trim(value);  },number:true },
                    'pcm_total':{normalizer: function(value) { return $.trim(value);  },number:true },
                    'pcm_percentage':{ normalizer: function(value) { return $.trim(value);  },number:true },
                    ...
                    'category_1':{normalizer: function(value) { return $.trim(value);  },number:true },
                    'course_1':{normalizer: function(value) { return $.trim(value);  },number:true },
...
//        }, "year required");
	 ...
	 $(document).on('change','#p_address_for_communication', function(event) {
        ...
        $(document.body).on("click",".sslc_add_btn",function()
        {
             
            $(".sslc_table_row:last").clone().hide().appendTo('.sslc_table_main').show('slow'); //taken clone of the last
            $(".sslc_table_row:last").find('input[type=text]').val('');
            $(".sslc_table_row:last").find('input[type=text]').removeClass('error');
            $(".sslc_table_row:last").find(".error").remove(); 
            $(".sslc_table_row:last").find(".validation-error-label").remove(); 
            
            $('.sslc_table_row').each(function(index){ 
                var index = parseInt(index)+1;
                $(this).attr('id','sslc_table_row_'+index);
                $(this).find(".sslc_subject").attr('name','sslc_subject['+(index)+']'); 
                $(this).find(".sslc_grade").attr('name','sslc_grade['+(index)+']'); 
                $(this).find(".sslc_remove_btn").attr('data-row',index); 
                $(this).find(".sslc_subject").rules("add", {required:true, normalizer: function(value) { return $.trim(value);  } } );
                $(this).find(".sslc_grade").rules("add", {required:true, normalizer: function(value) { return $.trim(value);  } } );
                if(index !==1)
                {
                    $(this).find(".sslc_remove_btn").css("display", "inline-block");
                }else{
              ...
              if(RowRemove!=1){ 
                setTimeout(function(){ 
                    ...
                    $('.table_row').each(function(index){
                        ...
                        $(this).find(".grade").attr('name','grade['+(index)+']'); 
                         if(index ==1){
                             $(this).find(".subject").rules('remove', 'required'); 
                             $(this).find(".mark_obtained").rules('remove', 'required'); 
                             $(this).find(".maximum_marks").rules('remove', 'required'); 
                             $(this).find(".grade").rules('remove', 'required'); 
     ...
     $(document.body).on("click",".sslc_remove_btn",function()
        {
              var RowRemove = $(this).attr('data-row');
              if(RowRemove!=1){ 
                setTimeout(function(){ 
                    $('#sslc_table_row_'+RowRemove).remove();
                    $('.sslc_table_row').each(function(index){
                        index = parseInt(index)+1; 
                        $(this).attr('id','sslc_table_row_'+index);
                        $(this).find(".sslc_remove_btn").attr('data-row',index);
                        ...
                        $(this).find(".sslc_grade").attr('name','sslc_grade['+(index)+']'); 
//                         if(index ==1){
//                             $(this).find(".sslc_subject").rules('remove', 'required'); 
//                             $(this).find(".sslc_grade").rules('remove', 'required'); 
//                             $(this).find(".invalid-feedback").remove(); 
//                         }
                    });
                }, 50);
