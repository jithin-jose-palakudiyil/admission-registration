...
<form  class="form-horizontal" id="BtechRegularForm" method="post" enctype="multipart/form-data" action="{{route('m_tech_store',\Crypt::encryptString($decrypted_forms_college_id))}}" class="form-horizontal">
    @csrf
    <div id="basicwizard"> 
        @include('applications::forms.m-tech.partials.nav-item') 
        <div class="tab-content b-0 mb-0 pt-0 BasicwizardTabs">
            @include('applications::forms.m-tech.form.personal-informations')
            @include('applications::forms.m-tech.form.family-informations')
            @include('applications::forms.m-tech.form.education-informations') 
            @include('applications::forms.m-tech.form.finish')
...
@section('js') 
    ...
    <script src="{{asset('Modules/Applications/Resources/assets/js/m-tech-form.js')}}"></script>
@endsection